...
import optax
import argparse
from m_class import Env, Optclasspl_traj
...
def producer(Gammas, effgradfn, solver, env):
    ...
    success = True
    Gammas0 = Gammas
    all_Gammas = jnp.zeros((env.max_iter, env.N-1)).at[0,:].set(Gammas0)
    (eta, grad) = effgradfn(Gammas)
    count = 0
    eta0 = eta
    all_etas = jnp.zeros((env.max_iter, 1)).at[0].set(eta0)
    opt = Optclasspl_traj(Gammas=Gammas, all_Gammas=all_Gammas, success=success, eta=eta, all_etas=all_etas, count=count, grad=grad, opt_state=opt_state)
    ...
    opt = jax.lax.cond(jnp.astype(opt.count, float) >= env.max_iter, lambda opt: Optclasspl_traj(Gammas = opt.Gammas, all_Gammas = opt.all_Gammas, success=False, eta=opt.eta, all_etas=opt.all_etas, count=opt.count, grad=opt.grad, opt_state=opt.opt_state), lambda opt: opt, opt)

    concat = jnp.concatenate((jnp.array([opt.count]), jnp.array([opt.success])))
    return concat, opt.all_Gammas, opt.all_etas
...
def main():
    ...
    if profile == 0:
        ...
        profile = "random"
        eps = jnp.array(args.Delta * np.random.rand(N))

    mystring = "global solver; solver = optax."+ args.optim + "(learning_rate=" + str(args.rate) + ")"
    exec(mystring)
    if args.exfix == 0:
        powers = sample_nd_grid(N, jnp.log10(minGg), jnp.log10(maxGg), points_per_dim, samples)
        f = lambda G: -1.*eff(10.0**G, ex, lk, gleak, eps, Jmax, alpha)
    else:
        powers = sample_nd_grid(N-1, jnp.log10(minGg), jnp.log10(maxGg), points_per_dim, samples)
        f = lambda G: -1.*eff(jnp.concatenate((jnp.array([args.exval]), 10.0**G)), ex, lk, gleak, eps, Jmax, alpha)
    ...
    result, all_Gammas, all_etas = prodvec(powers)
    colnames = []
    for i in range(N-1):
        for j in range(args.samples):
            colnames.append("Gamma"+str(i+2) + "_" + str(j+1))

    
    array = np.array(all_Gammas)
    array = np.concatenate([array[:,:,k].T for k in range(array.shape[2])], axis=1)
    df = pd.DataFrame(array, columns=colnames)
    df[colnames] = df[colnames].map(lambda x: 10.**x)

    colnames_eta = ["eta" + str(k+1) for k in range(args.samples)]
    etas = np.array(all_etas).squeeze()

    counts = result[:,0]
    successes = result[:,1]
    
    for k in range(args.samples):
        df.insert(len(df.columns), colnames_eta[k], etas[k,:])
    
    for k in range(args.samples):
        df.insert(len(df.columns), "count" + str(k+1), counts[k])
        df.insert(len(df.columns), "success" + str(k+1), successes[k])
    ...
    df.drop(index=np.arange(np.max(counts), max_iter), inplace=True)

    if args.exfix == 0:
        job_name = str(N) + "_" + no_decimal_string(Jmax) + "_" + no_decimal_string(alpha) + "_" + no_decimal_string(args.Delta) + "_" + profile + "pl" + "traj_"
    else:
        job_name = str(N) + "_" + no_decimal_string(Jmax) + "_" + no_decimal_string(alpha) + "_" + profile + "pl_exfix_" + "traj_"
    ...
    if args.save == True:
        if args.cluster == True:
            ...
            filepath = os.path.join(os.path.expanduser('~'), os.getcwd(), "results", "trajectories", job_name + dt_string + ".csv")
        df.to_csv(filepath)
