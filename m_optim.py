...
import optax
import argparse
from m_class import Env, Optclasspl
...
def producer(Gammas, effgradfn, solver, env):
    ...
    count = 0
    eta0 = eta
    opt = Optclasspl(Gammas=Gammas, success=success, eta=eta, count=count, grad=grad, opt_state=opt_state)
    ...
    opt = jax.lax.cond(jnp.astype(opt.count, float) >= env.max_iter, lambda opt: Optclasspl(Gammas = opt.Gammas, success=False, eta=opt.eta, count=opt.count, grad=opt.grad, opt_state=opt.opt_state), lambda opt: opt, opt)
...
def sample_nd_grid(ndim, lower_bounds, upper_bounds, num_points, num_samples):
    ...
    sampled_points = (upper_bounds-lower_bounds)*np.random.random(size=(num_samples,ndim)) + lower_bounds*np.ones((num_samples, ndim))
...
def main():
    ...
    if args.exfix == 0:
        job_name = str(N) + "_" + no_decimal_string(Jmax) + "_" + no_decimal_string(alpha) + "_" + no_decimal_string(args.Delta) + "_" + profile + "pl"
    else:
